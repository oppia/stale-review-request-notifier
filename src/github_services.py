...
def get_prs_assigned_to_reviewers(
    org_name: str, repo_name: str, max_waiting_time_in_hours: int
    ...
    while True:
        ...
        response = requests.get(
            pr_url, params=params, headers=_get_request_headers(), timeout=TIMEOUT_SECS
        )
...
def get_pull_request_object_from_dict(
    org_name: str, repo_name: str, pr_dict: Dict[str, Any]
    ...
    activity_url = ISSUE_TIMELINE_URL_TEMPLATE.format(org_name, repo_name, pr_number)
    ...
    while True:
        ...
        timeline_event_types.extend(event.get("event") for event in timeline_subset)
    ...
    return github_domain.PullRequest.from_github_response(updated_pr_dict)


# Here we use type Any because the response we get from the api call is hard
# to annotate in a typedDict.
def get_pull_request_dict_with_timestamp(
    pr_dict: Dict[str, Any], event: Dict[str, Any]
) -> Dict[str, Any]:
...
def _get_repository_id(
    ...
    variables = {"org_name": org_name, "repository": repo_name}
    ...
    repository_id: str = data["data"]["repository"]["id"]

    if repository_id is None:
        raise builtins.BaseException(f"{org_name}/{repo_name} doesn't exist.")

    return repository_id


@check_token
def _get_category_id(org_name: str, repo_name: str, discussion_category: str) -> str:
    ...
    discussion_categories = data["data"]["repository"]["discussionCategories"]["nodes"]
...
def delete_discussions(
    ...
    discussion_ids = _get_discussion_ids(org_name, repo_name, discussion_category)
